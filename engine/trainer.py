...
class Trainer(object):
    def __init__(self, cfg, mode='train'):
        ...
        if self.mode == 'train':
            print("train-self.dataset: ",self.dataset)
        ...
        if self.mode == 'eval':
            print("eval-self.dataset: ",self.dataset)
    ...
    def train(self, validate=False):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            # 没有建立验证集，注释掉，不然会报错 409-439
            # unless use data format with coco
            ...
            for outs in results:
                ...
                for i, im_id in enumerate(outs['im_id']):
                    ...
                    save_name = self._get_save_image_name(output_dir,
                                                          image_path)
                    logger.info("this Detection bbox results save in {}".format(
                        save_name))
        ...
        if visualize:
            for outs in results:
                ...
                for i, im_id in enumerate(outs['im_id']):
                    ...
                    save_name = self._get_save_image_name(output_dir,
                                                          image_path)
                    logger.info("预测结果保存至(Detection bbox results save in) {}".format(
                        save_name))
                    image.save(save_name, quality=95)
